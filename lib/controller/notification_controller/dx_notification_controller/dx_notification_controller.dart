...
class DxNotificationController extends GetxController {
  RxMap<String, List<NotificationContent>> deviceNotificationMapDx =
  ...
  Future<void> addNotification(
    ...
    if (!deviceNotificationMapDx.containsKey(deviceId)) {
      deviceNotificationMapDx[deviceId] = [];
    }
    deviceNotificationMapDx[deviceId]!.insert(0, content);
    deviceNotificationMapDx.refresh();
    await saveNotificationsToPrefs();
  }

  Future<void> clearNotifications(String deviceId) async {
    deviceNotificationMapDx[deviceId]?.clear();
    deviceNotificationMapDx.refresh();
    await saveNotificationsToPrefs();
  }

  Future<void> removeNotification(String deviceId, int index) async {
    deviceNotificationMapDx[deviceId]?.removeAt(index);
    deviceNotificationMapDx.refresh();
    await saveNotificationsToPrefs();
  }
  ...
  Future<void> saveNotificationsToPrefs() async {
    ...
    deviceNotificationMapDx.forEach((key, list) {
      encodedMap[key] =
          list.map((noti) => jsonEncode(_notificationToJson(noti))).toList();
    });

    await prefs.setString('deviceNotificationMapDx', jsonEncode(encodedMap));
  }

  Future<void> loadNotificationsFromPrefs() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? encodedData = prefs.getString('deviceNotificationMapDx');
    ...
    if (encodedData != null) {
      ...
      rawMap.forEach((key, value) {
        List<NotificationContent> list = (value as List)
            ...
            .toList();
        deviceNotificationMapDx[key] = list;
      });
      deviceNotificationMapDx.refresh();
    }
  }
