...
class SensorselectionChmB extends StatefulWidget {
  const SensorselectionChmB({super.key});

  @override
  State<SensorselectionChmB> createState() => _SensorselectionChmBState();
}

Timer? publishTimer;

class _SensorselectionChmBState extends State<SensorselectionChmB> {
          ...
          Row(
            ...
            children: [
              ...
              Padding(
                ...
                child: Obx(() {
                  final temps = [
                    _mqttController.sensorTemp1B.value,
                    _mqttController.sensorTemp2B.value,
                    _mqttController.sensorTemp3B.value,
                    _mqttController.sensorTemp4B.value,
          ...
          Row(
            ...
            children: [
              Obx(() {
                ...
                if (index == 0) {
                  selectedValue = _mqttController.selection1B.value;
                } else if (index == 1) {
                  selectedValue = _mqttController.selection2B.value;
                } else if (index == 2) {
                  selectedValue = _mqttController.selection3B.value;
                } else if (index == 3) {
                  selectedValue = _mqttController.selection4B.value;
                } 
                // else if (index == 4) {
                ...
                children: [
                  ...
                  IconButton(
                      onPressed: () {
                        ...
                        ctrl.dXMasterchangeOffsetB(index, -1);
                        publishTimer?.cancel();
                        publishTimer = Timer(Duration(seconds: 1), () {
                          _mqttController.buildJsonPayloadDMB();
                          publishTimer = Timer(Duration(seconds: 1), () {
                  ...
                  Obx(() {
                    final offsets = [
                      int.tryParse(_mqttController.offsett1B.value) ?? 0,
                      int.tryParse(_mqttController.offsett2B.value) ?? 0,
                      int.tryParse(_mqttController.offsett3B.value) ?? 0,
                      int.tryParse(_mqttController.offsett4B.value) ?? 0,
    ...
    Get.defaultDialog(
      ...
      content: Obx(() {
        ...
        final List<String> currentSelections = [
          _mqttController.selection1B.value,
          _mqttController.selection2B.value,
          _mqttController.selection3B.value,
          _mqttController.selection4B.value,
        ...
        return Column(
          ...
          children: List.generate(params.length, (optionIndex) {
            ...
            void setSelectionForSensor(int sensor, String value) {
              if (sensor == 0) {
                _mqttController.selection1B.value = value;
              } else if (sensor == 1) {
                _mqttController.selection2B.value = value;
              } else if (sensor == 2) {
                _mqttController.selection3B.value = value;
              } else if (sensor == 3) {
                _mqttController.selection4B.value = value;
              }
              //  else if (sensor == 4) {
