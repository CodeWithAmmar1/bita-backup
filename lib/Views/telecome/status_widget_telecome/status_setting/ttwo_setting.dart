...
class TtwoSetting extends StatelessWidget {
  TtwoSetting({super.key});
              ...
              () => Column(
                ...
                children: [
                  ...
                  SleekCircularSlider(
                    ...
                    max: 12,
                    initialValue:
                        _mqttController.tTwoTel.value.toDouble().clamp(0, 12),
                    onChangeStart: (_) =>
                        _mqttController.isUserInteracting.value = true,
                    onChange: (value) {
                      _mqttController.isUserInteracting.value = true;
                      _mqttController.tempTwoSetpoint(value);
                    },
                    onChangeEnd: (_) {
                      publishTimer?.cancel();
                      ...
                      children: [
                        ElevatedButton(
                          onPressed: () {
                            ...
                            if (_mqttController.tTwoTel.value > 0) {
                              _mqttController.tTwoTel.value -= 1;
                              publishTimer?.cancel();
                              publishTimer = Timer(Duration(seconds: 1), () {
                                _mqttController.tempTwoSetpoint(
                                    _mqttController.tTwoTel.value.toDouble());
                                _mqttController.isUserInteracting.value = false;
                              });
                            }
                          },
                        ...
                        ElevatedButton(
                          onPressed: () {
                            _mqttController.isUserInteracting.value = true;
                            if (_mqttController.tTwoTel.value < 12) {
                              _mqttController.tTwoTel.value += 1;
                              publishTimer?.cancel();
                              publishTimer = Timer(Duration(seconds: 1), () {
                                _mqttController.tempTwoSetpoint(
